...
* New function `purl_stata()`, the Stata analogue of `knitr::purl()`: extracts the code from the Stata chunks of an R Markdown or Quarto document into a do-file, recording each chunk's header (label and options) as a Stata comment, and following `knitr::purl()`'s `documentation` levels (`2` also carries the document text over as Stata comments). Chunks with `purl=FALSE` or `eval=FALSE` are skipped. (thanks @davidaarmstrong in #39)
